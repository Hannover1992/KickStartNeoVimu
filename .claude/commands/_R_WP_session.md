...
# /_R_WP_session - Session Parameter Configuration

```
╔══════════════════════════════════════════════════════════════════════════╗
║ VERTRAG: /_R_WP_session                                                 ║
╠══════════════════════════════════════════════════════════════════════════╣
║                                                                          ║
...
║                                                                          ║
║ NOTIFY:                                                                  ║
║   powershell -Command "notify 'WritePaper /_R_WP_session: {difficulty}'" ║
║                                                                          ║
║ TAGS:                                                                    ║
║   type: setup                                                            ║
║   op: WritePaper                                                         ║
...
- Quellen recherchieren (das macht /_R_WP_research)
- Kapitel-Struktur erstellen (das macht /_R_WP_structure)
- Paper-Inhalte generieren
- RAG-Queries durchfuehren

### Eskalation
- Bei unklarem difficulty-Input → Default: normal
- Bei fehlender _manifest.md → ERROR, User muss zuerst /_R_WP_init ausfuehren
- Bei fehlendem session-state.json → ERROR, User muss zuerst /_R_WP_init ausfuehren
...
**Validierung:**
- Wenn _manifest.md fehlt → STOP, User muss /_R_WP_init ausfuehren
- Wenn session-state.json fehlt → STOP, User muss /_R_WP_init ausfuehren
...
```
╔══════════════════════════════════════════════════════════════════════════╗
║ /_R_WP_session - Session Configuration                                  ║
╚══════════════════════════════════════════════════════════════════════════╝
...
### FEHLER, wenn
- [✗] _manifest.md fehlt → User muss /_R_WP_init ausfuehren
- [✗] session-state.json fehlt → User muss /_R_WP_init ausfuehren
...
Nach Abschluss:
```powershell
powershell -Command "notify 'WritePaper /_R_WP_session: {difficulty} configured, {agents_per_chapter} agents/chapter, temp {temp_min}-{temp_max}'"
```
