# /_R_WP_chapterModel - Chapter Knowledge Model
...
║ POSITION:                                                                ║
║   TYPE: LINEAR                                                           ║
║   AFTER: /_R_WP_assess (Coverage gut, >= 0.85)                          ║
║          /_R_WP_discovery (Luecken gefuellt, nach Gap-Loop)             ║
║   BEFORE: /_R_WP_chapterGap                                             ║
║   PHASE: Knowledge                                                      ║
...
**TUT NICHT (Claude):**
- RAG-Queries ausfuehren (das macht `/_R_WP_assess`)
- Neue Quellen suchen oder ingestieren (das macht `/_R_WP_research` + `/_R_WP_discovery`)
- Kapitel-Text schreiben (das macht `/_R_WP_write`)
- Luecken identifizieren oder bewerten (das macht `/_R_WP_chapterGap`)
...
- 0 Keywords in assessment.json → STOP + empfehle `/_R_WP_assess` wiederholen
...
2. Update `session-state.json`:
   ```json
   {
     "last_command": "_R_WP_chapterModel",
     "model_complete": true,
     "themes_count": 5,
     "references_count": 12,
     "references_valid": 10,
...
**Konsumenten:**
- `/_R_WP_chapterGap`: Prueft Model gegen Coverage-Anforderungen
- `/_R_WP_write`: Nutzt Themen + Referenzen + Schreib-Leitlinien fuer Kapitel-Text
