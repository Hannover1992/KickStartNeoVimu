...
**Status:** NEU v3.0 (Implementierungs-Pipeline Phase 1)
**Actor:** ARCHITEKT
...
║                                                                          ║
║  PIPELINE-POSITION:                                                      ║
║    [/_taskDefinition] → [/_spec] → [/_model] → [/_gap] → [/_I_cleanCodeArchitect] → [/_I_cleanCodeSlice] ║
║                                                                          ║
║  ACTOR: ARCHITEKT                                                        ║
...
```
TASKDEFINITION → MODEL → **CLEANCODEARCHITECT** → CLEANCODESLICE → CODEATOMIC
                                |
                                ↓
                          ARCHITECT.md
                          (Slices + Architektur)
...
**Prev:** /_model (Model.md erstellt/aktualisiert)
**Next:** /_I_cleanCodeSlice (Einzelner Slice planen)
...
Naechster Schritt: /_I_cleanCodeSlice {ERSTER_SLICE}
```
...
- [[I_cleanCodeSlice]] - Naechster Schritt (einzelnen Slice planen)
