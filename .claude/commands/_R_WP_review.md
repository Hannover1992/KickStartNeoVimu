# /_R_WP_review - Cross-Review & Scoring
...
║              │ AFTER: /_R_WP_convergence (Quality-Loop CONVERGED)         ║
║              │ BEFORE: /_R_WP_synthesis                                   ║
...
║              │ Error   → "✗ No drafts found, run /_R_WP_write first"     ║
╚════════════════════════════════════════════════════════════════════════════╝
```
...
- N Review-Agents parallel spawnen (M4 Parallel-Spawning)
...
- Drafts neu schreiben (das macht /_R_WP_write)
- Quality-Loop Entscheidungen treffen (das macht /_R_WP_convergence)
- Synthese ausführen (das macht /_R_WP_synthesis)
- Kapitel-Fortschritt steuern (das macht /_R_WP_orchestrate)
- Git-Operationen (user macht git selbst)
- MCP-Tools aufrufen (kein RAG in diesem Command)

### ESKALATION
- BLOCKING Error: Keine Drafts gefunden → NOTIFY Error + empfehle /_R_WP_write
...
**Ziel:** N Review-Agents parallel spawnen, jeder führt N-1 Reviews durch

**Aktionen:**
1. **Pro Review-Agent A{NN}** (parallel spawnen, M4 Pattern):
...
2. **Parallel-Spawning (M4):**
   - Spawne alle N Agents gleichzeitig
...
2. **Schreibe `output/reviews/chapter-{N}/review-summary.md`:**
   ...
   **Empfehlungen:**
   ...
   1. Run `/_R_WP_synthesis` to merge Top-{K} drafts
...
- [ ] N Review-Agents parallel gespawnt (M4)
...
✗ Cross-Review failed for Chapter {N}
  ...
  - Examples:
    - "No drafts found" → "Run /_R_WP_write first"
...
|-----------|---------|----------|--------|
| **BLOCKING** | 0 Drafts vorhanden | STOP + NOTIFY Error + empfehle /_R_WP_write | ✗ ERROR |
...
QUALITY-LOOP (bis CONVERGED):
  /_R_WP_visual → /_R_WP_convergence (prüft Konvergenz)
  → wenn RETRY → zurück zu /_R_WP_write (neue Drafts)
  → wenn CONVERGED → weiter zu /_R_WP_review (HIER)

POST-REVIEW:
  /_R_WP_review → /_R_WP_synthesis (Top-K mergen)
  → /_R_WP_reflect (Meta-Analyse)
  → zurück zu /_R_WP_orchestrate (nächstes Kapitel oder Feature-Ende)
```
...
- Blueprint: /_R_WP_write.md (Struktur-Vorlage)
- Previous: /_R_WP_convergence.md (Quality-Loop Entscheidung)
- Next: /_R_WP_synthesis.md (Top-K Merger)
