...
║     MAX-ITERATIONS: extern kontrolliert von /_WP_chapterGap (max 3)   ║
║   ENTRY:                                                                 ║
║     NORMAL: /_WP_research → THIS                                      ║
║   EXIT:                                                                  ║
║     NORMAL: THIS → /_WP_chapterModel                                  ║
║   PHASE: Knowledge                                                      ║
...
**DU TUST NICHT (NICHT):**
- Bestehende Quellen erneut ingestieren (das macht `/_WP_research`)
- Entscheiden ob Gap endgueltig geschlossen (das macht `/_WP_chapterGap`)
...
- Falls `research-gaps-initial.json` fehlt → FEHLER, empfehle `/_WP_assess` zuerst
```
...
5. Aktualisiere `output/references.bib`:
   ...
   - Markiere mit `note = {discovered via /_WP_discovery}`
...
3. Kein Gap geschlossen nach Discovery → Empfehle `/_WP_chapterGap` skip
