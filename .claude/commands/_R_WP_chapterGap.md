# /_R_WP_chapterGap - Chapter Gap Analysis & Decision
...
║   ENTRY:                                                                 ║
║     NORMAL: /_R_WP_chapterModel → THIS                                  ║
║   EXIT:                                                                  ║
║     PASS: THIS → /_R_WP_write (Coverage ausreichend)                    ║
║     GAP: THIS → /_R_WP_research (Luecken erkannt, Budget vorhanden)    ║
║     FORCE: THIS → /_R_WP_write (Budget erschoepft oder max Iterations) ║
║   PHASE: Knowledge                                                      ║
...
**TUT NICHT (Claude):**
- Luecken fuellen oder neue Quellen suchen (→ `/_R_WP_research` + `/_R_WP_discovery`)
- Kapitel-Text schreiben (→ `/_R_WP_write`)
- Wissens-Model aktualisieren (→ `/_R_WP_chapterModel`)
- Assessment wiederholen (→ `/_R_WP_assess`)
- RAG-Collection modifizieren

**ESKALATION:**
- chapter-{N}-model.md fehlt → STOP + empfehle `/_R_WP_chapterModel` zuerst
- assessment.json fehlt → STOP + empfehle `/_R_WP_assess` zuerst
...
**Konsumenten:**
- `/_R_WP_research`: Nutzt deficit_keywords fuer gezielte Quellen-Suche
- `/_R_WP_write`: Liest gap_severity fuer Schreib-Strategie (bei FORCE: Luecken markieren)
...
- chapter-{N}-model.md fehlt → BLOCKING STOP + ESKALATION, empfehle /_R_WP_chapterModel
- assessment.json fehlt → BLOCKING STOP + ESKALATION, empfehle /_R_WP_assess
...
- OUTER-Loop (Retry): Controller ist /_R_WP_reflect
- INNER-Loop (Quality): Controller ist /_R_WP_convergence
