...
║    4. .claude/models/{NAME}_Model.md  ◄── DATENBANK-MODUS (nur Listen)  ║
║       → W{n}-Status zaehlen (D2)                                         ║
║       → Offene-Bereiche-Liste lesen (D1, Kap. 6a)                       ║
║       → TC-Liste / Model-Topologie lesen (D4)                            ║
║    5. Vorheriges ERGEBNIS-Dokument  ◄── SRS-VORHER, Stagnations-Zaehler ║
║                                                                          ║
...
(\_SC_observe + \_SC_hypothese + \_SC_implement + \_SC_ergebnis). Interne Wellen (Drafter+Synthese)
...
| Schwierigkeit | Agents | Einsatz |
|---------------|--------|---------|
| **easy** | 1 (DU, Hauptagent) | Ergebnis offensichtlich, wenige Quellen |
| **normal** | 2-3 parallele Subagenten | Mehrere Quellen (Logs, Tests, Browser) |
| **hard** | 5-10 parallele Subagenten | Viele Container, umfangreiche Logs, komplexes Szenario |

**System-Model (aus Manifest):** Bestimmt welches Modell pro Welle laeuft.
- opus: Agents=sonnet, DU=sonnet (niedrig-kognitiv, kein opus noetig!)
- sonnet: Agents=haiku, DU=sonnet
- haiku: Agents=haiku, DU=haiku
