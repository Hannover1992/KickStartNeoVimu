...
║      W_PUSH_STATUS: COMPLETED ({Datum}, {Summary}) — Einzeiler      ║
║      PUSH_STATUS: COMPLETED (W10-Guard, bleibt State)               ║
║    SCHREIBT PROTOKOLL (_manifest_protokoll.md, nach Push):          ║
║      ## W_push_orchestrate [{Datum}]                                 ║
║      [Vollstaendiger Push-Log: Chunks, Collections, Sync-Status]    ║
║      Prepend-Mechanismus (W18): last_append + append_count++         ║
║                                                                      ║
║  SCHREIBT (Team Lead direkt):                                        ║
║    {VAULT}/_manifest.md  (nach jedem Step aktualisieren)             ║
║                                                                      ║
║  ARCHIVIERT (Team Lead direkt, Schritt 2a):                          ║
║    .claude/analysis/archive/{NAME}/exploration/{NAME}-*              ║
║    .claude/analysis/archive/{NAME}/drafts/{NAME}-*                   ║
║    Vorbedingung: synthese/{NAME}-GAP.md existiert                    ║
║    Semantik: Move (kein Delete), Idempotent, Feature-Prefix-Match   ║
║    NICHT archiviert: synthese/ (TC-4, INV-03)                        ║
║                                                                      ║
...
|-----------|---------|-------|-------------|
| `NAME` | (Manifest) | String | Feature-/Forschungsname. Falls nicht angegeben: 4-Stufen-Fallback (siehe unten). |
...
IF NAME nicht als Argument angegeben:
  Lies /home/uczen/Documents/DCS/OmniCommand/_manifest.md
...
- Manifest existiert: `/home/uczen/Documents/DCS/OmniCommand/_manifest.md`
...
1. `/home/uczen/Documents/DCS/OmniCommand/_manifest.md` → NAME, Phase, Zyklen-Info
...
1. Lade den Command via Skill-Tool:
   Skill(skill="{COMMAND_NAME}", args="{NAME}")
   Beispiele:
     Skill(skill="_W_obsidianSync", args="DCSRE-31 normal")
     Skill(skill="_W_push_temp", args="DCSRE-31")
     Skill(skill="_retrospektive", args="DCSRE-31")
...
2. TeamDelete
2a. # (ProzessKorrektur P7, RF-21/RF-22) Archive-Hook: Exploration/Drafts archivieren
    # Vorbedingung: Synthese muss existieren (Dateien sind "verbraucht")
    IF .claude/analysis/synthese/{NAME}-GAP.md existiert:
      # Feature-Prefix-Match: NUR {NAME}-* Dateien verschieben (INV-08)
      # Zielstruktur: analysis/archive/{NAME}/exploration/ und analysis/archive/{NAME}/drafts/
      archive_base = .claude/analysis/archive/{NAME}
      erstelle_verzeichnis(archive_base/exploration/)
      erstelle_verzeichnis(archive_base/drafts/)

      # Exploration archivieren
      exploration_dateien = glob(.claude/analysis/exploration/{NAME}-*)
      archiviert_count = 0
      bereits_vorhanden_count = 0
      FOR EACH datei IN exploration_dateien:
        ziel = archive_base/exploration/{dateiname}
        IF ziel existiert bereits:
          bereits_vorhanden_count += 1
          SKIP  # Idempotenz (RF-22)
        ELSE:
          verschiebe(datei, ziel)  # Move, KEIN Delete (W31)
          archiviert_count += 1

      # Drafts archivieren
      drafts_dateien = glob(.claude/analysis/drafts/{NAME}-*)
      FOR EACH datei IN drafts_dateien:
        ziel = archive_base/drafts/{dateiname}
        IF ziel existiert bereits:
          bereits_vorhanden_count += 1
          SKIP
        ELSE:
          verschiebe(datei, ziel)
          archiviert_count += 1

      Logge: "[ARCHIVE-HOOK] {archiviert_count} archiviert, {bereits_vorhanden_count} bereits vorhanden."
      # HINWEIS: synthese/ wird NICHT archiviert (TC-4, INV-03)
    ELSE:
      Logge: "[ARCHIVE-HOOK] SKIP — Keine synthese/{NAME}-GAP.md gefunden (Synthese nicht abgeschlossen)."

3a. Push-Log Protokoll-Write (Pattern C, W18):
    Schritt 3a.1: Frontmatter _manifest_protokoll.md aktualisieren:
      last_append: {Datum}
      append_count: {N+1}
    Schritt 3a.2: Prepend nach YAML-Frontmatter in _manifest_protokoll.md:
      ## W_push_orchestrate [{Datum}]
      - Model: {NAME}_Model.md finalisiert + gesplittet
      - RAG: global_knowledge gepusht (Chunks: {N}, Collections: {Liste})
      - Vault: synchronisiert ({difficulty}, Co-Working-Links gesetzt)
      - Retrospektive: dokumentiert
3b. State-Update _manifest.md (Einzeiler):
    W_PUSH_STATUS: COMPLETED ({Datum}, Model+RAG+Vault+Retro)
...
Description: |
  ...
  Input: .claude/models/{NAME}_Model.md, /home/uczen/Documents/DCS/OmniCommand/Task.md
...
Description: |
  ...
  Input: /home/uczen/Documents/DCS/OmniCommand/_manifest.md,
...
| Vault nicht erreichbar | WARNING-Guard (via obsidianSync v2.2): FRAGE bei HiL=on, STOP bei HiL=off ohne DEGRADED_MODE. Nur RAG push bei DEGRADED MODE. |
