# /_R_WP_visual - Visual Figure Generation & Review
...
│ 4. POSITION     = AFTER /_R_WP_write, BEFORE /_R_WP_qualityGate             │
...
│                   Error: "output/drafts/ fehlt, run /_R_WP_write first"     │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
### ESKALATION
- BLOCKING: output/drafts/chapter-{N}/ fehlt → Error + empfehle /_R_WP_write
...
1. Erstelle Human-Readable-Report:
   ...
   OPTIONEN:
   [A] APPROVE   - Weiter zu /_R_WP_qualityGate
...
2. **_manifest.md UPDATE:**
   ...
   - Section "## WritePaper Progress" aktualisieren:
     ```markdown
     - [x] /_R_WP_write (Kapitel 1)
     - [x] /_R_WP_visual (Kapitel 1, 4/6 Figures)
     - [ ] /_R_WP_qualityGate (Kapitel 1)
     ```
...
3. **session-state.json UPDATE:**
   ```json
   {
     ...
     "hil_decision": "APPROVE",
     "visual_iteration": 1,
     "last_command": "/_R_WP_visual",
     "timestamp": "2026-02-09T14:30:00Z"
   }
   ```

4. **NOTIFY:**
   - Success: "✓ Kapitel {N} Figures: {X}/{Y} generiert. Next: /_R_WP_qualityGate"
...
- Pipeline bereit für /_R_WP_qualityGate
...
|-----------|---------|----------|--------|
| **BLOCKING** | output/drafts/chapter-{N}/ fehlt | STOP + Error-NOTIFY + Empfehle /_R_WP_write | JA (ERROR) |
| **BLOCKING** | Keine Draft-Dateien vorhanden | STOP + Error-NOTIFY + Empfehle /_R_WP_write | JA (ERROR) |
...
10. **Next Step:** Immer /_R_WP_qualityGate nach APPROVE/SKIP
