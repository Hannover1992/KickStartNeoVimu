...
# /_R_WP_init - WritePaper Projekt-Initialisierung
...
║                                                                          ║
║ NOTIFY:                                                                  ║
║   powershell -Command "notify 'WritePaper /_R_WP_init abgeschlossen'"   ║
║                                                                          ║
║ TAGS:                                                                    ║
║   type: setup                                                            ║
║   op: WritePaper                                                         ║
...
### TUT NICHT
- **Keine Quellen ingestieren**: Das macht `/_R_WP_research` später
- **Keine Struktur extrahieren**: Das macht `/_R_WP_structure` später
...
## Status
- **Phase**: Setup
- **Next Step**: /_R_WP_session (Session-Planning)
...
WENN alle Checks ✓:
  ...
  → Ausgabe: "➜ Nächster Schritt: /_R_WP_session"
  → NOTIFY: "WritePaper /_R_WP_init abgeschlossen"
  → EXIT 0
...
➜ Nächster Schritt: /_R_WP_session
...
**Chain-Flow**:
```
[START] → /_R_WP_init → [/_R_WP_session]
```
