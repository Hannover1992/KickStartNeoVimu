...
require('lazy').setup({
  ...
  -- Set SONAR_QUBE_ENABLED=1 in environment to enable
  {
    ...
    config = function()
      require('sonarqube').setup({
        ...
        csharp = {
          enabled = true,
          omnisharpDirectory = vim.fn.expand('~/AppData/Local/nvim-data/mason/packages/omnisharp/'),
        },
      })
    end,
  },
  ...
  -- Neogit - Magit for Neovim (Git UI)
  {
    ...
    opts = {
      -- Performance optimizations for large repos (especially Windows + /mnt/c/)
  ...
  -- Neotest - Modern test runner for Neovim
  {
    ...
    config = function()
      require('neotest').setup({
        adapters = {
          -- C#/.NET adapter - Simple configuration for Windows with Docker Desktop
...
vim.keymap.set('n', '<leader>yw', function()
  ...
  -- In Windows native, path is already in Windows format
  vim.fn.setreg('+', current_file)
  vim.notify('Copied Windows path: ' .. current_file, vim.log.levels.INFO)
